...
```{r}
ggplot( data =mpg) + geom_point(mapping = aes(x = displ, hwy, color = displ < 5))
```

```{r}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy, color = shape = class)) +
...
```{r}
# Find a builtin data set for students to explore; use their cheat sheet from stats to label continuous and discrete data; (come up with 3 graphs). Use graphs to tell me things about 3 data sets. 

(nycflights13::flights)
```

```{r}
# For HOMEWORK, please refer to file titled: ggplot2_HWonly.rmd
```
