...
```{r}
names <- c("John", "Jim", "Jake", "Jan", "Jenny")
names
```
...
student_names <- c("John", "Jim", "Jake", "Jan", "Jason", "Jenny")
...
3.  Print the third row of the first matrix.
    ...
    ```{r}
    Thirdrow_firstmatrix <- matrix(1:9, nrow = 3, ncol = 3)
    Thirdrow_firstmatrix[3,]
    ```

4.  Print the second column of both matrices.

    ```{r}
    first_matrix <- matrix(c(1,2,3,4,5,6,7,8,9), nrow = 3, byrow = TRUE)
    second_matrix <- matrix(c(9,8,7,6,5,4,3,2,1), nrow = 3, byrow = TRUE)
